...
fn pack_sync_end_to_end() {
    ...
    assert!(widget.is_file(), "pack template must write src/widget.rs under the project");
    assert!(!project.join("generated").exists(), "no generated/ dir allowed");
    let widget_src = std::fs::read_to_string(&widget).expect("widget");
    assert!(widget_src.contains("pub struct Widget"), "widget body: {widget_src}");
    assert!(widget_src.contains("pub name: String"), "widget fields: {widget_src}");
    assert!(widget_src.contains("pub count: String"), "widget fields: {widget_src}");
    // Local project template also ran, driven by the union graph.
    let instances =
        std::fs::read_to_string(project.join("out/instances.txt")).expect("instances");
    ...
    assert!(lock.contains("source = \"path:../demo-pack\""), "lock: {lock}");
    let expected_hash = first.packs.get("widget").expect("pack hash in report");
    assert_eq!(expected_hash.len(), 64, "blake3 hex");
    assert!(lock.contains(&format!("content_hash = \"blake3:{expected_hash}\"")), "lock: {lock}");

    // Report decisions bind why each file landed.
    assert_eq!(first.decisions.get("src/widget.rs").map(String::as_str), Some("written"));
    ...
    assert_eq!(
        receipt.payload.decisions.get("src/widget.rs").map(String::as_str),
        Some("written")
    );
    ...
    assert_eq!(lock2, lock, "ggen.lock must be byte-identical across identical runs");
    ...
    let receipt2: SyncReceipt =
        serde_json::from_slice(&receipt2_raw).expect("receipt 2 parses");
    let receipt3: SyncReceipt =
        serde_json::from_slice(&receipt3_raw).expect("receipt 3 parses");
    assert_eq!(
...
fn tampered_receipt_decision_fails_verification() {
    ...
    std::fs::write(&receipt_path, serde_json::to_vec_pretty(&json).expect("ser"))
...
fn dry_run_never_writes_or_mutates_lock() {
    ...
    assert!(!project.join("ggen.lock").exists(), "dry run must not write ggen.lock");
    assert!(!project.join("src/widget.rs").exists(), "dry run must not write outputs");
    assert!(!project.join(RECEIPT_REL_PATH).exists(), "dry run must not emit a receipt");
...
#[test]
fn git_pack_ref_is_typed_not_implemented() {
    let (_dir, project) = scaffold();
    let manifest = project.join("ggen.toml");
    let toml = std::fs::read_to_string(&manifest).expect("ggen.toml");
    let toml = toml.replace(
        "widget = { path = \"../demo-pack\" }",
        "widget = { git = \"https://example.com/pack.git\", version = \"1.0.0\" }",
    );
    std::fs::write(&manifest, toml).expect("rewrite");

    let err = sync(&project, SyncOptions { dry_run: false }).expect_err("git must refuse");
    let msg = err.to_string();
    assert!(msg.contains("FM-PACK-007"), "{msg}");
...
fn broken_packs_refuse_by_name() {
    ...
    let (_dir, project) = scaffold();
    std::fs::remove_file(project.parent().expect("root").join("demo-pack/ontology.ttl"))
        .expect("rm ttl");
    ...
    let (_dir, project) = scaffold();
    std::fs::remove_file(
        project.parent().expect("root").join("demo-pack/templates/widget.rs.tmpl"),
    )
    .expect("rm tmpl");
...
fn two_packs_disjoint_outputs_both_succeed() {
    let dir = TempDir::new().expect("tempdir");
    write_pack(dir.path(), "pack-a", "Alpha", "src/alpha.rs", "pub const ALPHA: &str = \"a\";");
    write_pack(dir.path(), "pack-b", "Beta", "src/beta.rs", "pub const BETA: &str = \"b\";");
    ...
    assert_eq!(report.decisions.get("src/alpha.rs").map(String::as_str), Some("written"));
    assert_eq!(report.decisions.get("src/beta.rs").map(String::as_str), Some("written"));

    let lock = std::fs::read_to_string(project.join("ggen.lock")).expect("ggen.lock");
    let idx_a = lock.find("[packs.pack-a]").expect("pack-a entry in lock");
    let idx_b = lock.find("[packs.pack-b]").expect("pack-b entry in lock");
    assert!(idx_a < idx_b, "ggen.lock must list packs alphabetically: {lock}");

    assert!(report.packs.contains_key("pack-a"), "report.packs: {:?}", report.packs);
    assert!(report.packs.contains_key("pack-b"), "report.packs: {:?}", report.packs);
...
fn two_packs_colliding_output_aborts_sync_without_rollback_or_lock() {
    ...
    assert!(!project.join("ggen.lock").exists(), "ggen.lock must not be written");
    assert!(!project.join(RECEIPT_REL_PATH).exists(), "receipt must not be written");
}
