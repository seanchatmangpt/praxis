...
fn wasm4pm_compat_pack_syncs() {
    ...
    assert!(events.is_file(), "pack must write src/wasm4pm_compat_events.rs");
    let events_src = std::fs::read_to_string(&events).expect("events.rs");
    assert!(events_src.contains("pub enum EmittedEventType"), "{events_src}");
    assert!(events_src.contains("GraphUnionHashed"), "{events_src}");
    assert!(events_src.contains("\"graph_union_hashed\""), "{events_src}");
    assert!(events_src.contains("pub fn emit_pack_lock_verified"), "{events_src}");
    assert!(
    ...
    let doc = std::fs::read_to_string(project.join("docs/wasm4pm_compat_emission.md"))
        .expect("emission doc");
    for name in ["graph_union_hashed", "pack_lock_verified", "receipt_chained"] {
...
fn lsp_max_pack_syncs() {
    ...
    assert!(toml.contains("id = \"LSPMAX-UNWRAP-001\""), "rule toml: {toml}");
    assert!(toml.contains("severity = \"error\""), "rule toml: {toml}");
    assert!(toml.contains(r"pattern = '\.unwrap\(\)|\.expect\('"), "rule toml: {toml}");
    assert!(
        toml.contains("RulePackServer handlers return ClassifiedFindings"),
        "rationale must flow from the ontology: {toml}"
    );
    assert!(
        project.join("rules/lsp_max_lspmax_rawjson_002.toml").is_file(),
        "second rule file missing"
    );
    assert!(
        project.join("rules/lsp_max_lspmax_wallclock_003.toml").is_file(),
        "third rule file missing"
    );
    let index = std::fs::read_to_string(project.join("docs/lsp_max_rule_pack.md"))
        .expect("rule index doc");
    ...
    assert_eq!(before, after, "second sync must leave the rule file unchanged");
}
...
fn star_toml_pack_syncs() {
    ...
    assert!(src.contains("pub witness_dir: std::path::PathBuf"), "module: {src}");
    assert!(src.contains("pub fail_closed: bool"), "module: {src}");
    assert!(src.contains("pub exporter_endpoint: String"), "module: {src}");
    ...
    assert!(src.contains("#[serde(deny_unknown_fields)]"), "module: {src}");
    assert!(src.contains("star_toml::load_file::<Self>(path)"), "module: {src}");
    ...
    assert!(admission_doc.contains("`witness_dir`"), "doc: {admission_doc}");
    let telemetry_doc = std::fs::read_to_string(project.join("docs/star_toml/telemetry.md"))
        .expect("telemetry doc");
    assert!(telemetry_doc.contains("`sample_rate`"), "doc: {telemetry_doc}");
...
fn chicago_tdd_tools_pack_syncs() {
    ...
    assert!(boundary.is_file(), "pack must write tests/chicago_tdd_tools_boundary.rs");
    let src = std::fs::read_to_string(&boundary).expect("boundary tests");
    assert!(src.contains("use chicago_tdd_tools::cli_proof::CliHarness;"), "{src}");
    assert!(src.contains("fn receiptctl_help_lists_verbs()"), "{src}");
    assert!(src.contains("fn receiptctl_version_emits_name()"), "{src}");
    assert!(src.contains("fn receiptctl_unknown_verb_fails_closed()"), "{src}");
    assert!(src.contains("CliHarness::cargo_bin(\"receiptctl\")"), "{src}");
    ...
    // (3) ggen.lock records the pack by name with a blake3 hash.
    let lock = std::fs::read_to_string(project.join("ggen.lock")).expect("ggen.lock");
    assert!(lock.contains("[packs.chicago-tdd-tools-pack]"), "lock: {lock}");
    assert!(lock.contains("content_hash = \"blake3:"), "lock: {lock}");
...
fn clap_noun_verb_pack_syncs() {
    ...
    assert!(routes_path.is_file(), "pack must write src/clap_noun_verb_routes.rs");
    ...
    assert!(
        routes.contains(r#"#[clap_noun_verb_macros::verb("verify", "session")]"#),
    ...
    let doc = std::fs::read_to_string(project.join("docs/clap_noun_verb_routes.md"))
        .expect("routes doc");
    assert!(doc.contains("| `session` | `login` | `session_login_handler` |"), "doc: {doc}");
    ...
    assert_eq!(routes, routes_again, "second sync must leave the route file unchanged");
}
...
fn praxis_core_pack_syncs() {
    ...
    assert!(rs_path.is_file(), "pack must write src/praxis_core_refusal_table.rs");
    ...
    assert!(rs.contains("pub const PRAXIS_CORE_REFUSAL_TAXONOMY"), "{rs}");
    assert!(
