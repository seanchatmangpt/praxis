...
fn repo_translation_001_loads() {
    ...
    let code_blocks: Vec<_> = user
        .blocks
        .iter()
        .filter(|b| {
            matches!(
                b.kind,
                rust_fable_testbed::spec::PromptBlockKind::Code { .. }
            )
        })
        .collect();
...
fn repo_translation_001_target_path_is_describe_rs_not_lib_rs() {
    let ttl_path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tasks/repo_translation_001.ttl");
    let task = rust_fable_testbed::spec::load_task(&ttl_path).expect("load_task should succeed");
    assert_eq!(
        task.target_path,
        Some(std::path::PathBuf::from("src/describe.rs"))
    );
}
