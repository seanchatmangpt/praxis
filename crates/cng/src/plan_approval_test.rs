...
test!(
    present_plan_derives_real_datalog_roles_for_a_non_bench_roster_artifact,
    {
        ...
        let roster_ttl_path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/live-roster.ttl");
        let roster_ttl =
            fs::read_to_string(&roster_ttl_path).expect("read live-roster.ttl fixture");
...
// EDS falsifier: the Datalog role-inference decision made inside
// `present_plan` must itself be auditable/receipted, not just an in-memory
// value handed back to the caller and discarded. Prove it by reading the
// raw `plan-ledger.jsonl` line back off disk (bypassing `PresentedPlan`
// entirely) and asserting the persisted `Presented` event carries the same
// `roster_roles`/`roster_obligations` the caller observed.
#[cfg(feature = "role-inference")]
test!(
    present_plan_persists_datalog_roles_into_the_receipted_ledger_line,
    {
        let dir = render_fixture("live_roster_roles_receipted", 9);
        let ledger_dir = ledger_scratch_dir("live_roster_roles_receipted");
        let roster_ttl_path =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/live-roster.ttl");
        let roster_ttl =
            fs::read_to_string(&roster_ttl_path).expect("read live-roster.ttl fixture");
        ...
        assert!(
            ...
            "fixture must actually carry roster facts for this falsifier to be meaningful"
        );

        // Bypass PresentedPlan entirely: read the raw JSONL ledger line back off
        // disk and parse it as untyped JSON, the same way an external auditor
        // (not this process, not this in-memory struct) would inspect the
        // receipted admission trail.
        let ledger_path = ledger_dir.join("plan-ledger.jsonl");
        let ledger_text = fs::read_to_string(&ledger_path).expect("read plan-ledger.jsonl");
        let line = ledger_text
            .lines()
            .find(|l| l.contains("\"presented\""))
            .expect("ledger must contain a presented event line");
        let raw: serde_json::Value = serde_json::from_str(line).expect("ledger line is valid JSON");

        let ledger_roles = raw.get("roster_roles").and_then(|v| v.as_object()).expect(
            "persisted ledger line must carry roster_roles, not just the in-memory return value",
        );
        assert_eq!(
        ledger_roles.get("worker-avery").and_then(|v| v.as_str()),
        Some("reviewer"),
        "the Datalog admission decision for worker-avery must be receipted in the ledger line, \
         matching the in-memory value ({:?})",
        presented.roster_roles.get("worker-avery")
    );

        let ledger_obligations = raw
            .get("roster_obligations")
            .and_then(|v| v.as_object())
            .expect("persisted ledger line must carry roster_obligations");
        assert_eq!(
            ledger_obligations
                .get("worker-avery")
                .and_then(|v| v.as_str()),
...
#[cfg(feature = "role-inference")]
test!(
    present_plan_yields_no_roster_roles_for_a_plain_pddl_only_artifact,
    {
