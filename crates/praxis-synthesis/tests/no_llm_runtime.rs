...
fn dependencies_are_exactly_the_offline_allowlist() {
    ...
    for raw in manifest.lines() {
        ...
        if in_deps && !line.is_empty() && !line.starts_with('#') {
            let name = line.split('=').next().expect("split yields at least one piece").trim();
...
fn source_contains_no_llm_symbols() {
    ...
    for entry in fs::read_dir(&src).expect("src/ readable") {
        ...
        files += 1;
        let text = fs::read_to_string(&path).expect("source readable").to_lowercase();
    ...
    assert!(files > 20, "expected the full src/ module set, scanned only {files} files");
}
