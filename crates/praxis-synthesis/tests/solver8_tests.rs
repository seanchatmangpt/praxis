...
fn saturated() -> (praxis_synthesis::Program, Vec<praxis_synthesis::Capability>, Vec<praxis_synthesis::Atom>)
{
...
fn differential_solver8_equals_boundedcsp_without_constraints() {
    ...
    let problem =
        SequenceProblem::new(&p, caps, goal, 6, Vec::new()).expect("problem");
...
fn differential_holds_under_satisfiable_constraints() {
    let (p, caps, goal) = saturated();
    let constraints = vec![
        Constraint::Before { a: "judge".into(), b: "receipt".into() },
        Constraint::NotEarlier { a: "admit".into(), k: 3 },
    ...
    let admit_at = plan.steps.iter().position(|s| s.capability == "admit").expect("admit");
    assert!(admit_at >= 3);
}
...
fn conflicting_windows_produce_a_minimal_core_naming_the_culprits() {
    ...
    let constraints = vec![
        Constraint::Before { a: "supply-evidence".into(), b: "clear-obligations".into() }, // decoy
        Constraint::NotLater { a: "receipt".into(), k: 2 },
        Constraint::NotEarlier { a: "admit".into(), k: 3 },
        Constraint::Before { a: "admit".into(), b: "receipt".into() },
        Constraint::AtMost { a: "judge".into(), n: 1 }, // decoy
    ];
    let problem =
        SequenceProblem::with_constraints(&p, caps, goal, 6, constraints).expect("problem");
    ...
    assert!(core.contains(&"NotLater(receipt,2)".to_string()), "core: {core:?}");
    assert!(core.contains(&"NotEarlier(admit,3)".to_string()), "core: {core:?}");
    assert!(core.contains(&"Before(admit,receipt)".to_string()), "core: {core:?}");
    // Decoys excluded — the core is minimal.
    assert!(!core.iter().any(|c| c.contains("supply-evidence")), "core: {core:?}");
...
fn the_core_alone_reverifies_without_search() {
    ...
    let problem = SequenceProblem::with_constraints(&p, caps, goal, 6, core.clone())
        .expect("problem");
    assert!(verify_core(&problem, &core), "certificate checks by propagation alone");
    // Dropping any single member breaks the certificate (minimality).
    for i in 0..core.len() {
        let mut sub = core.clone();
        sub.remove(i);
        assert!(!verify_core(&problem, &sub), "proper subset must not certify");
    }
}
...
fn core_cache_replays_the_dead_end_for_the_fleet() {
    ...
    let e1 = Solver8.solve_cached(&problem, &mut cache).expect_err("unsat");
    assert!(matches!(e1, Refusal::UnsatProof { replayed: false, .. }));
    assert_eq!(cache.len(), 1);
    // Every later agent replays it — no propagation, no MUS, no search.
    let e2 = Solver8.solve_cached(&problem, &mut cache).expect_err("unsat");
...
fn excludes_forbids_coexistence() {
    ...
    let constraints =
        vec![Constraint::Excludes { a: "judge".into(), b: "admit".into() }];
    let problem =
        SequenceProblem::with_constraints(&p, caps, goal, 6, constraints).expect("problem");
    assert!(matches!(
...
fn requires_is_enforced_at_goal() {
    ...
    let constraints =
        vec![Constraint::Requires { a: "shortcut".into(), b: "audit".into() }];
    let problem =
    ...
    assert_eq!(plan.cost, 2, "shortcut+audit (2) beats long-route (10): {names:?}");
}
