...
fn end_to_end_lawobject_synthesis_is_admitted() {
    ...
    assert_eq!(
        receipt.plan_steps, 5,
        "the discovered plan is the 5-step lawobject pipe"
    );
    assert!(receipt.verdict.ok);
    assert_eq!(receipt.verdict.checks.len(), 6, "all six refinements ran");
    assert!(
        receipt.verdict.checks.iter().all(|c| c.ok),
        "{:?}",
        receipt.verdict
    );
    ...
    assert_eq!(
        receipt.chain.len(),
        64,
        "one blake3 chain for the whole run"
    );
}
...
fn warm_cache_run_is_fully_memoized_and_same_chain() {
    ...
    assert_eq!(
        warm.dag.replayed_count,
        warm.dag.node_receipts.len(),
        "100% replayed"
    );
    assert_eq!(
        cold.chain, warm.chain,
        "replayed run produces the identical receipt"
    );
}
