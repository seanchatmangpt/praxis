...
%% NOTE: transition/2 currently returns a bare context() (S'), not the
%% {S', C} pair (next state plus a finite command/consequence set) that
%% PRD v26.7.11 section 7.7 requires (delta_AIR: (S,E) -> (S',C)), and
%% there is no AND/join dependency-readiness support yet (advancement is a
%% per-step unconditional bitmask). Both are tracked as PROJ-755/PROJ-756
%% (docs/jira/v26.7.11/tickets/index.md) rather than fixed here.
