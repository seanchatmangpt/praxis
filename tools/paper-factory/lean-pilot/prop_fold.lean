...
h_n is a deterministic function of (Genesis, fr_1, ..., fr_n), computable by a
single left fold; equal inputs give bit-identical h_n.

Bare Lean 4 core, no mathlib. Reuses `Chain`, `chainCommitments`,
`terminalCommitment`, `Frame`, `genesis` from def:chain verbatim.
-/
...
/-- The sequence of commitments `h_0, h_1, ..., h_n` for a chain, computed by a
    single left fold starting from `Genesis`. -/
...
/-- **prop:fold**: `h_n` is a deterministic function of `(Genesis, fr_1, ..., fr_n)`,
    computed by a single left fold (`chainCommitments`/`terminalCommitment` are
    ordinary total functions of the chain `l`); consequently equal input chains give
    bit-identical terminal commitments `h_n`. -/
