...
DISCLOSURE: this file declares an unproven, empirical placeholder axiom. It
is not a mathematical proposition and no proof is possible or intended.
...
Anything downstream that imports `Praxis.Corpus.RefCurve` (which re-exports
this file) and cites `noveltyCurveUnderFaultsWithheld` is citing an axiom, not
a theorem: the witness below asserts the marker Prop holds by fiat, with no
proof term and no empirical measurement backing it in this repository. Do not
present it as "proven" downstream without carrying this caveat forward.
-/
...
/-- UNPROVEN, by design: this witness is an opaque axiom, not a theorem. It
records that the withheld-claim marker is treated as holding "by
construction" (this file's existence documents the refusal itself -- member
-level injection recovery was measured; node-level injection was not, so the
stronger claim is not asserted) -- it is not derived from any proof term or
from actual node-level fault-injection data, because no such data exists yet
in this repository. -/
