...
fn admit(matrix: &mut DfCmMatrix, source: &str, socket: &str, executed: bool, fixture: &str) {
    let cell = matrix
...
fn refuse(matrix: &mut DfCmMatrix, source: &str, socket: &str, reason: &str, salvage: &str) {
    let cell = matrix
        .find_cell_mut(&[source, socket])
        .unwrap_or_else(|| panic!("cell ({source}, {socket}) not in expanded matrix"));
...
#[must_use]
pub fn build_frontier_matrix() -> DfCmMatrix {
    let axes = vec![
        DfCmAxis {
...
#[must_use]
pub fn evaluated_count() -> usize {
    build_frontier_matrix().evaluated()
}
...
#[must_use]
pub fn frontier_report() -> DfCmReport {
    DfCmReport::from_matrix(&build_frontier_matrix())
}
...
#[must_use]
pub fn full_report() -> FrontierReport {
...
pub fn write_report(path: &Path) -> io::Result<FrontierReport> {
