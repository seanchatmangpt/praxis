...
| G12 | `engine serve` loop + Arazzo/API projection on lawful path | PARTIAL | Arazzo leg real and digest-verified; OpenAPI/AsyncAPI leg not wired into dispatch (see MISSING #1). |
...
| `NoBeneficialDecomposition` | ⚠️ No dedicated forced test. Only appears inside `A | B` loose matches (`cng_decomp.rs:105-118`) or "one of three strings" membership checks (`cng_ipc_corpus.rs:164-179`, `cng_ipc_corpus_full_scale.rs:82-92`) that never confirm this specific branch fired. |
...
gaps last. This is a reporting artifact only — no item below has been acted on.
