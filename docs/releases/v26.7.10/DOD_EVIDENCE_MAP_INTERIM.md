# DOD_EVIDENCE_MAP_INTERIM — Clause-by-Clause Evidence Map for v26.7.10 (interim)

SUPERSEDED (2026-07-10): this is the prior v26.7.10 (interim) evidence map, preserved verbatim
(fix-forward, not deleted) per PROJ-748. It indexes evidence for the superseded interim DoD
(`DEFINITION_OF_DONE_INTERIM.md`). The governing evidence map for v26.7.10-revised is
`DOD_EVIDENCE_MAP.md` in this directory (PROJ-748).
...
`DOD_SIGNOFF_INTERIM.md` (PROJ-617): per the consolidated final build cited there, all 11
success markers derived TRUE via SPARQL. PLANNED rows below are the pre-build planning record,
kept for traceability; where they disagree with `DOD_SIGNOFF_INTERIM.md`, the sign-off wins.
...
- **Query** — the SPARQL file under `crates/cng/queries/` expected to prove the marker over the
  ...
  `(PLANNED)` = named by `DEFINITION_OF_DONE_INTERIM.md` or ticket scope but not yet real
  evidence.
- **Refusal** — the `CngRefusal` variant guarding the clause. Variant doc comments and stable
  ...
  `forced_false_marker_refuses_cng_r20`); final verdicts live in `DOD_SIGNOFF_INTERIM.md`.
...
Note on naming (resolved at PROJ-617 closure): the `DEFINITION_OF_DONE_INTERIM.md` §4 names
are authoritative and on disk — `metric-hook-actuations.rq` and `metric-dispatch-closure.rq`
...
`DEFINITION_OF_DONE_INTERIM.md` §2, clauses 1-12. All PLANNED as behaviors (PROJ-608..622);
mechanism column names the intended enforcement, not achieved state.
...
- `docs/releases/v26.7.10/DEFINITION_OF_DONE_INTERIM.md` — the doctrine this map indexes
- `docs/releases/v26.7.10/DOD_EVIDENCE_MAP.md` — governing map for v26.7.10-revised (PROJ-748)
