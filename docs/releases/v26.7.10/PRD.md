...
Status: CLOSED. All five v26.7.10 scope tickets (PROJ-601..605) landed and verified this
session. Every claim cites a file, test, or receipt. Rows without evidence are marked UNKNOWN,
never asserted.

This release covers the `cng` benchmark surface (`crates/cng/src/bench.rs`, `bench` cargo
feature) at commit `e763f44` plus the v26.7.10 hardening work (bundle-manifest completeness
PROJ-603, auditor-replay portability PROJ-601, the `cng evidence replay` auditor verb PROJ-602,
inline-SPARQL closure PROJ-604, the `CNG_R11 AuditMismatch` refusal variant PROJ-605) — all now
ALIVE, per rows 2, 3, 5, 8, and 13 below.
...
| 2 | Run is independently reproducible/replayable | ALIVE, scoped | `cng benchmark verify --dir <dir>` exit 0, `REPLAY_RESULT=3/3` — a sample, not full-corpus replay; row 3 closed this ticket's portability gap | `crates/cng/src/bench.rs` `verify()` | PROJ-601 (CLOSED) |
| 3 | Third-party auditor can move the bundle directory and replay it elsewhere | ALIVE | Fixed: `run()` writes `bench_dir`-relative `digests.json` keys (refusing `HardcodingSuspicion` on escape); `verify()` rejoins against its own `--dir`, refusing `AuditMismatch`/`CNG_R11` on missing resolved paths. Proved this session: `cp -R X Y && rm -rf X`, `just cng-bench-verify Y` → exit 0, `REPLAY_RESULT=3/3`, identical to the pre-move result | `crates/cng/src/bench.rs` `run()`/`verify()` digest-key construction; `crates/cng/tests/cng_bench_portability.rs` (2 tests) | PROJ-601 (CLOSED) |
| 4 | `benchmark verify` re-derives and checks all evidence digests (OCEL graph, SPARQL results) | PARTIAL, unchanged by design | `verify()` still re-derives the POWL manufacture digest only; full-evidence-graph re-derivation is now a separate, independent path (row 13, `cng evidence replay`), not folded into `verify()` — this is a deliberate scope split, not a regression | `crates/cng/src/bench.rs` `verify()` | — |
| 5 | No inline Turtle/SPARQL in Rust source | ALIVE | Closed: the 7 remaining sites (`pipeline.rs:135`, `shape.rs` ×6) converted to `include_str!("queries/*.rq")` + `str::replace`, byte-identical substituted queries; guard test replaced with whole-src `no_inline_sparql_in_rust_sources`, passing over all of `src/`+`tests/` | `crates/cng/src/queries/*.rq` (7 files); `crates/cng/tests/no_inline_ttl_guard.rs::no_inline_sparql_in_rust_sources` | PROJ-604 (CLOSED) |
...
| 8 | Bundle manifest names every input/output digest in one file | ALIVE | Closed: `EvidenceManifest` (obs/query/ontology/rules digests + `ocel_graph_digest`/`sparql_result_digest`/`evidence_chain_digest`, unpopulated `signatures: []`) written to `<bench_dir>/results/evidence-manifest.json`; bundle self-contained via copied `queries/`, `ontology/`, `rules/bench-roles.dl`. `query_digests.len() >= 15` asserted in test | `crates/cng/src/bench.rs` `EvidenceManifest`, `QuerySet::digests()`; `crates/cng/tests/cng_bench_portability.rs::evidence_manifest_is_complete_and_relative` | PROJ-603 (CLOSED) |
| 13 | An independent third party can verify a relocated bundle's OCEL evidence graph without any producer state | ALIVE | New `cng evidence replay --bundle <dir>` verb: parses the bundle manifest, recomputes `obs_digest` + every query digest + the full OCEL evidence digest from bundle files alone, compares to the manifest. Proved this session on the relocated bundle `Y`: `just cng-evidence-replay Y` → `AUDIT_RESULT=CONFORMANT`, exit 0, all three match flags true. Negative proof: tampering one `obs/*.ttl` byte (a comment append, not a parse break) → `CNG_R11: obs digest mismatch`, exit 1 | `crates/cng/src/bench.rs` `audit_replay()`, `AuditReplayReport`; `crates/cng/src/main.rs` `evidence_replay` (`#[verb("replay","evidence")]`); `justfile` `cng-evidence-replay`; `crates/cng/tests/cng_bench_portability.rs` (3 `audit_replay_*` tests) | PROJ-602 (CLOSED) |
