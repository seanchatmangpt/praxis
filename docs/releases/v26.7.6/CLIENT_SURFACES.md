...
| Browser shell (out of scope) | `/Users/sac/dashboard.bak` (pqc-evidence-center) | Nuxt 4.0.3 | **RESOLVED_BY_SCOPE_RECLASSIFICATION** — 88 real pages (threat-intel, incident-response, security-compliance, govern/measure/manage), Supabase-wired, but the code requires Nuxt UI Pro (paid license, external dependency): 93 `UDashboard*` usages (`grep -rc 'UDashboard' app`) and `app/assets/css/main.css` imports `@nuxt/ui-pro`. Not a git repo. Not release-critical — autonomic-platform + optimus carry the release-critical client evidence. | no git |
| **Browser shell (canonical Nuxt)** | `/Users/sac/wasm4pm/apps/playground-web` | Nuxt 4.4.6 | **RESOLVED_BY_EXISTING_SURFACE (build PASS)** — canonical Nuxt shell. `pnpm --filter nuxt-ui-template-dashboard run build` from the wasm4pm root completed 2026-07-06 (exit 0, `.output/` 19.6 MB / 6 MB gzip) after `pnpm install --filter @wasm4pm/observability --filter nuxt-ui-template-dashboard` (exit 0). `@nuxt/ui` ^4.8.2 (no Pro license), committed `playwright.config.ts`; caveat: in-app `pnpm run build` fails via nested `pnpm-workspace.yaml` scoping — build from root with `--filter`. | 2026-07-05 |
...
| Build command per client (executed 2026-07-06) | **optimus: PASS** — `next build` completed (route table emitted, static+dynamic pages, middleware 35.4 kB) despite npm-install fallback noise. **pcp: BLOCKED_TYPED** — `npx expo export --platform web` fails: unresolved modules `@/src/components/AvatarRelativeProjection` (from `src/app/(auth)/_layout.tsx`) and `@/src/hooks/useOcelEvidence` (from `(tabs)/process.tsx`) — source files missing from repo; next action: restore/stub the missing modules or point imports at existing components. **dashboard.bak: BLOCKED_TYPED** — `nuxi build` fails: dependency-generation skew. `app/assets/css/main.css` is authored for Nuxt UI Pro v3 / Tailwind v4 (`@import "tailwindcss" theme(static)`, `@theme static`, `@import "@nuxt/ui-pro"`) while `package.json` declares `@nuxt/ui` ^2.21 (Tailwind v3). No git history to recover the matching pair. Next action (adoption decision, not build fix): upgrade to `@nuxt/ui` v3 + ui-pro (license required) OR re-home the 88 pages onto the in-tree `wasm4pm/apps/playground-web` shell (Nuxt 4.4.6, committed). Resolution 2026-07-06: dashboard.bak `RESOLVED_BY_SCOPE_RECLASSIFICATION` (Nuxt UI Pro = paid external dependency; not release-critical); playground-web promoted canonical, **build PASS** — `pnpm --filter nuxt-ui-template-dashboard run build` from wasm4pm root, exit 0 (see `NO_TERMINAL_BLOCKERS.md`). |
