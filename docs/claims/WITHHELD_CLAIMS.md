# Withheld claims register

Claims praxis deliberately does NOT make. Each entry names what would be
required before the claim could move to the proven column. Append-only:
entries are removed only by a receipted retraction, never edited away.
