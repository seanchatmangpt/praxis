...
about the interior it summarizes~\cite{chatman2025}. % [CITE]
This paper supplies the cryptography
...
folds each frame onto its causal predecessor. % [CODE]
We prove, from an explicit collision-resistance
...
$O(1)$ per frame and $O(\CL)$ at the boundary. % [BOUNDARY]
We prove the \textbf{Conservation of
...
collision, and each artifact carries \emph{at least one} committed admitted cause. % [BOUNDARY]
We give
...
stage that caught it. % [CODE]
We formalize Ed25519 signing as \emph{self-contained authenticity},
...
its index-scale query direction. % [TEXT]
We close with the \textbf{Integrity--Virtue Scope Theorem}:
a proof of what a receipt chain establishes and, equally rigorously, what it does not --- the
guardrail that keeps a cryptographic apparatus from being mistaken for a warrant of goodness. % [TEXT]
...
it summarizes.'' This paper pays the debt. % [BOUNDARY]
We construct the object that discharges the
assumption --- the receipt chain --- and prove, rather than assert, exactly which lies it
forecloses and which it does not. % [BOUNDARY]
A receipt is often imagined as a \emph{copy}: a smaller transcript of what happened, trusted
because it looks like the original. That picture is wrong at every scale that matters. % [TEXT]
A copy
is faithful only if the reader compares it to the original, which is precisely the
comprehension cost the keystone showed we cannot pay. % [TEXT]
The receipt in this doctrine is not a
copy but a \emph{commitment}: a short value $h_{+}$ such that no adversary can exhibit a
different history yielding the same $h_{+}$ without breaking a cryptographic assumption. % [BOUNDARY]
The
verifier never compares the receipt to the interior. % [BOUNDARY]
It recomputes one arithmetic step and
checks equality. % [TEXT]
Fidelity is enforced by the algebra of the hash, not by the diligence of the
reader. % [TEXT]
...
\item \textbf{Integrity.} Was this history altered after the fact? (Answered by the chain.) % [TEXT]
\item \textbf{Authenticity.} Who vouches for this history? (Answered by a signature.) % [TEXT]
...
(Q2); Chapter~\ref{ch:scope} proves the boundary of (Q3). % [BOUNDARY]
Keeping these separate is not
pedantry. % [TEXT]
A system that answers (Q1) and pretends it has answered (Q3) is exactly the
overclaim the keystone's antibody clause forbids~\cite{chatman2025}. % [CITE]
...
\code{praxis\_core::signing}. % [CODE]
Where a size, a field order, or a constant appears below, it is
the size, field order, or constant the code compiles with, and in several cases the code
asserts it at compile time. % [TEXT]
...
and $h_{-},h_{+}$ the chain values before and after a step. % [BOUNDARY]
The keystone's law-bearing frame
is here made concrete as the byte-exact \code{OcelCausalFrame}. % [CODE]
...
If two distinct histories can
serialize to the same bytes, the hash of those bytes commits to neither unambiguously. % [TEXT]
The
...
prefix, because the layout is fixed. % [TEXT]
The praxis frame achieves this by being a
\code{repr(C, align(64))} struct of a single compile-time-constant size. % [CODE]
...
--- two $64$-byte cache lines --- with the field widths of Table~\ref{tab:layout}. % [DEF]
The
in-memory layout carries $5$ bytes of interior padding (\code{pad}) so that the $256$-bit
\code{prior\_hash} lands aligned; the padding is structural, not semantic. % [DEF]
\end{definition}

\begin{remark}
The $128$-byte size is not a description but an invariant the compiler enforces. % [TEXT]
The crate
contains the const assertion % [TEXT]
\[
\code{const \_: () = \{ assert!(size\_of::<OcelCausalFrame>() == 128) \};}
\]
so a change to any field width that broke the two-cache-line layout would fail to compile. % [TEXT]
Fixed width is thus a mechanized property (in the sense of the foundations paper's
mechanized-totality results), not a convention a maintainer must remember. % [TEXT]
\end{remark}
...
little-endian in a fixed order. % [TEXT]
This is the object the chain actually commits to. % [TEXT]
...
by \code{to\_hash\_bytes}, with the field layout of Table~\ref{tab:layout}. % [DEF]
It is a total,
injective function of the semantic fields of $\fr$ (all fields except the structural
\code{pad}): distinct field tuples yield distinct bodies. % [DEF]
\end{definition}
...
\bottomrule
\end{tabular}
\caption{The $99$-byte canonical hash body $\body(\fr)$ produced by \code{to\_hash\_bytes}. % [CODE]
The $5$ interior \code{pad} bytes of the $128$-byte in-memory frame are \emph{not} hashed. % [CODE]
Total committed width: $8+8+8+32+8+2+1+32=99$ bytes.} % [TEXT]
\label{tab:layout}
\end{table}
...
$\fr'$ agree on every field of Definition~\ref{def:frame}. % [THM]
\end{proposition}

\begin{proof}
Each field occupies a disjoint, fixed byte range (Table~\ref{tab:layout}), and each
integer field's little-endian encoding is a bijection on its width. % [PROOF]
A byte-equal body forces
range-wise equality, hence field-wise equality. % [PROOF]
The \code{pad} bytes are absent from the
body, so they do not participate; two frames differing only in padding have equal bodies,
which is correct --- padding carries no consequence. % [PROOF]
 \end{proof}
...
for. % [TEXT]
The frame has no dedicated ``payload'' field; instead it carries eight packed
object-reference words \code{obj\_refs[0:8]}, each a \code{u32}, normally OCEL object handles. % [CODE]
The praxis frame-builder repurposes all eight words to hold the $256$-bit BLAKE3 digest of the
canonical payload bytes, \emph{verbatim}. % [TEXT]
...
words $w_0,\dots,w_7$ and set $\code{obj\_refs}[i]=\code{PackedObjRef}(w_i)$. % [DEF]
Crucially the
...
bits. % [DEF]
The raw constructor preserves all $32$ bits, so the full $256$-bit digest survives
into bytes $[24,56)$ of $\body(\fr)$ intact. % [DEF]
\end{construction}
...
Proposition~\ref{prop:bodyinj} any change to $\dg(p)$ changes $\body(\fr)$. % [THM]
Consequently a
...
distinct payload digests induce distinct bodies. % [PROOF]
 \end{proof}
...
already serializes all eight words verbatim. % [CODE]
This is a recurring discipline in the codebase
...
a fixed-width coordinate (its $256$-bit digest inside a $99$-byte body). % [TEXT]
\end{remark}
...
A single frame commits one step. % [TEXT]
A \emph{chain} commits an ordered history by folding each
frame's body onto the running commitment of everything before it. % [TEXT]
...
zero bytes. % [DEF]
A run may additionally bind a \emph{domain seed}
...
For a frame $\fr$ with predecessor commitment $h_{-}$, the successor commitment is % [DEF]
\begin{equation}\label{eq:chain}
h_{+} \;=\; \chainH\bigl(h_{-}\concat\body(\fr)\bigr),
\end{equation}
where $\concat$ is byte concatenation. % [DEF]
A \emph{chain} (ledger) $\Ledger=(\fr_1,\dots,\fr_n)$
has commitments $h_0=\Genesis$ and $h_t=\chainH(h_{t-1}\concat\body(\fr_t))$ for
$1\le t\le n$; its terminal value is $h_n$. % [DEF]
\end{definition}
...
\eqref{eq:chain}. % [CODE]
This double-mixing predates the current construction and is retained for
compatibility. % [TEXT]
It neither strengthens nor weakens the security argument below --- $h_{+}$
remains a deterministic function of $h_{-}$ and $\fr$ --- so all theorems hold verbatim
whether the predecessor is mixed once or twice. % [BOUNDARY]
We note it only so a reader diffing the code
against \eqref{eq:chain} is not surprised. % [TEXT]
\end{remark}
...
(\Genesis,\ (\fr_t)_t)$. Equal inputs give bit-identical $h_n$. % [THM]
\end{proposition}

\begin{proof}
Immediate from \eqref{eq:chain}: each step is a pure function of two arguments, and
composition of pure functions is pure. % [PROOF]
BLAKE3 is a fixed function, so no nondeterminism
enters. % [PROOF]
 \end{proof}

Proposition~\ref{prop:fold} has an operational corollary that the codebase enforces
structurally. % [TEXT]
Emission (minting a receipt at manufacture time) and replay (recomputing it
later from stored fields) must agree, or tamper detection would raise false positives. % [TEXT]
The
crate guarantees agreement by routing both through one function. % [TEXT]
...
\code{ReceiptRecord::recompute\_chain\_hash}. % [THM]
Both call the same \code{build\_admission\_frame}
and \code{chain\_from\_frame}. % [THM]
Hence for identical stored fields they compute identical
$h_{+}$; any disagreement between a stored \code{chain\_hash\_hex} and its recomputation is
therefore attributable to a change in a stored field, never to divergent code paths. % [THM]
\end{proposition}
...
that folds it. % [PROOF]
Both callers invoke these with the record's own fields
(\code{payload\_hash}, \code{prev\_chain\_hash}, \code{ReceiptMeta}, \code{ts\_ns}). % [PROOF]
Function
purity (Proposition~\ref{prop:fold}) then forces equal outputs on equal inputs, so a
recompute mismatch isolates a field mutation rather than a logic difference. % [PROOF]
 \end{proof}
...
change to any of them changes $h_t$ except with negligible probability. % [THM]
In particular the
terminal $h_n$ commits the entire history. % [THM]
\end{proposition}

\begin{proof}
By induction on $t$. % [PROOF]
Base: $h_0=\Genesis$. % [PROOF]
Step: $h_t=\chainH(h_{t-1}\concat\body(\fr_t))$
...
The collision
clause is deferred to Theorem~\ref{thm:faithful}. % [PROOF]
 \end{proof}

\section{Git-as-a-Runtime CAS Locking and Ledgers}

To prevent coordination drift in local-first, air-gapped environment execution, we rely on the host repository's reference structures to enforce atomic locks. % [TEXT]

\begin{definition}[Git CAS Lock]\label{def:gitlock}
An atomic Compare-And-Swap (CAS) lock for resource $L$ at repository HEAD OID $H$ is acquired by invoking the reference update: % [DEF]
\begin{equation}\label{eq:gitlock}
\code{git update-ref refs/locks/}L\ H\ \bm 0,
\end{equation}
which fails with a non-zero exit code if $\code{refs/locks/}L$ already exists, and succeeds atomically otherwise via POSIX rename. % [DEF]
Deletion on drop releases the lock. % [DEF]
\end{definition}

\begin{proposition}[Append-Only notes Ledger]\label{prop:notesledger}
The audit ledger is stored under the custom namespace $\code{refs/notes/praxis/audit}$. % [THM]
Appending entries is a commit transition in which the hash-chain receipts are written as NDJSON notes, preserving the immutability of the execution timeline. % [THM]
\end{proposition}
\begin{proof}
...
Concurrently, the acquisition of a Git CAS lock (Definition~\ref{def:gitlock}) ensures that only one writer can atomically perform the reference transition:
$\code{git update-ref refs/locks/}L\ H\ \bm 0$
prevents race conditions, ensuring that concurrent writers cannot silently overwrite or split histories. % [PROOF]
...
probability $\varepsilon(\lambda)$ in the security parameter $\lambda$. % [AX]
For BLAKE3,
$\lambda=256$ and the best generic attack is the birthday bound $\sim 2^{128}$. % [AX]
\end{axiom}
...
Construction~\ref{con:commit}). % [THM]
If $\Ledger'$ has the \emph{same} terminal commitment $h_n$,
then a collision of $\chainH$ has been exhibited. % [THM]
Consequently, under Axiom~\ref{ax:cr}, an
adversary who alters any committed field yet preserves $h_n$ succeeds only with probability
$\le \varepsilon(\lambda)$. % [THM]
\end{theorem}
...
Because $j$ is least, the
predecessors agree: $h_{j-1}=h'_{j-1}$. % [PROOF]
Consider the two fold inputs
$x=h_{j-1}\concat\body(\fr_j)$ and $x'=h'_{j-1}\concat\body(\fr'_j)$. % [PROOF]
Since the prefixes
...
$t\ge j$ to map unequal inputs to equal outputs --- a collision. % [PROOF]
If instead $h_j=h'_j$ despite
$x\ne x'$, then $\chainH(x)=\chainH(x')$ is itself a collision at step $j$. % [PROOF]
In either case
$h_n=h'_n$ forces a collision, contradicting Axiom~\ref{ax:cr} up to $\varepsilon(\lambda)$. % [PROOF]
\end{proof}
...
and of the interior dimension of the manufacture the frame receipts. % [THM]
Checking the boundary
projection of a whole run (verdict, terminal $h_n$, fitness scalar, refusal reason) is
$O(\CL)$, matching the keystone's Comprehension--Verification Gap. % [THM]
\end{corollary}

\begin{proof}
The fold reads two fixed-width operands and produces a $256$-bit output; BLAKE3 on a
fixed-length input is constant work. % [PROOF]
...
applying the boundary predicate is $O(\CL)$. % [PROOF]
Neither cost references $n$ or $\dim\Sig$. % [PROOF]
\end{proof}

\begin{remark}[Faithful \emph{only} over committed fields]
Theorem~\ref{thm:faithful} is deliberately scoped to committed fields. % [TEXT]
A change confined to a
...
non-hashed --- does not change $h_n$ and is, correctly, \emph{not} attested. % [CODE]
Faithfulness is a
property of the commitment, and the commitment is exactly Table~\ref{tab:layout}. % [TEXT]
This is the
...
digest, and the transition identity attests those. % [TEXT]
What you commit is what you can trust; no
more, and no less. % [TEXT]
\end{remark}

\chapter{Conservation of Consequence}
\label{ch:conservation}

The keystone stated Conservation of Consequence abstractly. % [ORIENT]
Here it becomes a theorem about
chain positions, provable directly from Definitions~\ref{def:chain} and the Chatman equation. % [TEXT]
...
for an object that reached the \code{Admitted} stage. % [THM]
\item \textbf{Positioned.} Each actuation appends \emph{exactly one} frame and advances the
chain by \emph{exactly one} commitment; the map $\Psi:\text{actuation}\mapsto h_{+}$ is
injective up to hash collision. % [THM]
...
bytes $[16,24)$ commit the denial word certifying $x$ passed admission. % [THM]
\end{enumerate}
\end{theorem}
...
In the code, \code{receipt} is a method
...
\code{chain\_hash}; \eqref{eq:chain} appends exactly one commitment per frame. % [PROOF]
Suppose two
distinct actuations mapped to the same $h_{+}$. % [PROOF]
Their frames differ (distinct instruction
identity in \code{instruction\_id}, or distinct payload digest), so by
Theorem~\ref{thm:faithful} equal terminal commitments force a collision. % [PROOF]
Hence $\Psi$ is
injective up to $\varepsilon(\lambda)$. % [PROOF]
...
persists an admitted-polarity frame). % [PROOF]
Thus the frame commits both the identity of the cause
and the verdict that admitted it. % [PROOF]
 \end{proof}
...
position is shared by two actuations (\emph{no double-count}: by (ii)). % [THM]
Consequence is
neither created without an admitted cause nor recorded without a unique receipt slot. % [THM]
\end{corollary}
\begin{proof}
...
manufacture the same artifact. % [TEXT]
Clause (iii) does not claim $a$ determines $x$; it claims the
frame \emph{commits} $\dg(x)$, so $h_{+}$ pins the cause even where the artifact alone would
not recover it. % [TEXT]
Uniqueness of the cause is a property of the receipt, not a false property of
the morphism. % [TEXT]
\end{remark}
...
Theorem~\ref{thm:faithful} gives a \emph{detector}: a tampered ledger fails to verify. % [TEXT]
A
production verifier must do more --- it must say \emph{where} and \emph{how} the ledger is
broken, because at fleet scale ``this ledger is invalid'' is an alarm with no address. % [BOUNDARY]
The
praxis \code{ReceiptValidator} is a staged pipeline that localizes the fault. % [CODE]
...
outcomes rather than short-circuiting: % [DEF]
\begin{enumerate}[label=(S\arabic*),leftmargin=*]
\item \code{schema} --- every record has the supported version and well-formed
$32$-byte hex fields (\code{payload\_hash}, \code{prev\_chain\_hash}, \code{chain\_hash}). % [DEF]
\item \code{chain\_recompute} --- for each $t$, \code{recompute\_chain\_hash} equals the stored
\code{chain\_hash\_hex}. % [DEF]
This is the direct instantiation of Theorem~\ref{thm:faithful}: a
mismatch is a committed-field tamper. % [DEF]
...
non-decreasing, and no \code{ts\_ns} exceeds the (injectable) clock's ``now''. % [DEF]
\item \code{token\_replay} --- each record replays through the fixed POWL token model to
fitness exactly $\code{0x0001\_0000}$ (\S\ref{sec:fitness}). % [DEF]
\end{enumerate}
\end{definition}
...
first affected record is index $j$. % [THM]
Then the pipeline of Definition~\ref{def:pipeline} reports
...
class of the fault. % [THM]
The honest prefix $\fr_1,\dots,\fr_{j-1}$ is not implicated. % [THM]
\end{theorem}
...
unperturbed and recompute equal. % [PROOF]
...
predecessor's \code{chain\_hash}. % [PROOF]
...
Chapter~\ref{ch:auth}) makes suffix rewriting a collision problem. % [PROOF]
...
both). % [ORIENT]
This mirrors the affidavit-style \code{verify} pipeline's ``run every stage, report all
...
is the constant $\code{0x0001\_0000}=65536$. % [DEF]
The \code{token\_replay} stage accepts a record
iff its replayed fitness equals $\code{0x0001\_0000}$. % [DEF]
\end{definition}
...
nonconformant step, and the stage fails naming that record. % [THM]
\end{proposition}
...
consumption, making the numerator positive and $\Fitness<1$, at the earliest offending index. % [PROOF]
...
prior value. % [BOUNDARY]
Authenticity is added by binding the terminal commitment to a keyholder. % [TEXT]
...
signature~\cite{ed25519} over it, and $k$ is the hex verifying key. % [DEF]
It is \emph{self-contained}:
...
There are two verification predicates: % [THM]
\begin{itemize}[leftmargin=*]
\item \textbf{Integrity} (\code{verify\_chain\_hash}): check $s$ against the \emph{embedded}
key $k$ and that $k$'s signature covers the presented $h$. % [THM]
This proves the signed object was
not altered after signing, but trusts whatever key is inside it. % [THM]
\item \textbf{Authenticity} (\code{verify\_chain\_hash\_with\_key}): check $s$ against an
\emph{externally pinned} key $k^{\star}$. % [THM]
This fails if the receipt was signed by any key other
than $k^{\star}$, even if internally consistent. % [THM]
\end{itemize}
The first is necessary but not sufficient for the second; a verifier who wants ``this came from
the party I trust'' must use the second and supply $k^{\star}$ from an independent source. % [THM]
\end{proposition}
...
establish authorship. % [PROOF]
\code{verify\_chain\_hash\_with\_key} instantiates the key argument with
...
holder of $sk^{\star}$. % [PROOF]
The embedded-key check therefore cannot substitute for the pinned-key
check. % [PROOF]
 \end{proof}
...
path returns \code{SigningFailed} rather than emitting an unsigned receipt. % [THM]
\end{proposition}

\begin{proof}
The \code{signed} feature exists to \emph{guarantee} a signature; silently skipping would
defeat its purpose and let an unsigned receipt masquerade as covered. % [PROOF]
The implementation
therefore treats key absence as failure. % [PROOF]
Authenticity is thus either present and checkable or
explicitly absent, never ambiguously assumed. % [PROOF]
 \end{proof}
...
$h_n$ as the anchored value. % [BOUNDARY]
Neither alone suffices against a resourceful adversary; together
they reduce forgery to breaking BLAKE3 collision resistance \emph{or} Ed25519 unforgeability. % [BOUNDARY]
\end{remark}

\chapter{The Receipt Lake: Content-Addressed and Queryable at Index Scale}
\label{ch:lake}

Individual receipts chain into a per-run ledger. % [TEXT]
Across a fleet, ledgers accumulate into a
\emph{receipt lake} $\Lake$: a content-addressed, append-only store engineered so that the
population of receipts is queryable without reading interiors. % [TEXT]

\begin{definition}[Content addressing]\label{def:contentaddr}
The address of a stored object $b$ is $\code{content\_address}(b)=\chainH(b)$ in hex. % [DEF]
Two
byte-identical objects share an address; any change yields a different address. % [DEF]
A receipt's
...
old bytes: history is immutable by construction, and references are stable. % [THM]
\end{proposition}

\begin{proof}
(a) The address is $\chainH(b)$, a function of $b$; equal $b$ gives equal address. (b) A change
$b\to b'$ with $b\ne b'$ gives $\chainH(b')\ne\chainH(b)$ except on a collision
(Axiom~\ref{ax:cr}); the reference $\chainH(b)$ therefore cannot be made to resolve to $b'$. % [PROOF]
\end{proof}

\section{OCEL 2.0 and PROV-O mappings}

A receipt is not only a hash; it carries process structure. % [TEXT]
Each \code{ReceiptRecord} names the
OCEL objects it governs via \code{object\_ids} (event-to-object links), an \code{activity\_idx}
and \code{node\_kind} placing it in a POWL process, and an \code{instruction\_id} sequencing it. % [CODE]
This lets the ledger export as a standard event log. % [TEXT]
...
\code{object\_ids}. % [DEF]
The result is a conformant OCEL~2.0 log queryable by any process-mining
tool, produced by \code{receipt export-ocel}. % [DEF]
\end{construction}
...
a \code{prov:wasDerivedFrom} edge to the predecessor. % [DEF]
The praxis \code{receipt\_shacl} module
...
\emph{shape-checked} against a published contract before it enters the lake. % [DEF]
\end{construction}
...
guarantee --- a consumer can recompute and re-verify from the exported form. % [THM]
\end{proposition}
...
and \code{prev\_chain\_hash}, which the record retains. % [PROOF]
Hence the exported log determines $h_n$,
and re-verification is Theorem~\ref{thm:faithful} applied to the reconstructed frames. % [PROOF]
\end{proof}
...
graph. % [TEXT]
An engineered SPARQL index such as QLever~\cite{qlever} answers queries over
$10^{11}$-scale triple stores by a join plan no engineer reconstructs --- the very
``differential agreement over a checked projection'' the keystone cites as manufactured trust. % [CITE]
...
$O(\CL)$-sized committed coordinates, never the interiors. % [TEXT]
This is the projection principle at
the scale of a whole civilization's audit log: the interior is a trillion manufactures; the
queryable surface is their committed frames. % [TEXT]
\end{remark}
...
receipt lock problem onto the host Git object store. % [CODE]
For an air-gapped, local-first
...
POSIX-compliant filesystems. % [TEXT]
...
HEAD OID of the local repository. % [DEF]
The CAS lock for $L$ is acquired by % [DEF]
\[
...
absent''). % [DEF]
Deletion of the reference on drop releases the lock. % [DEF]
The lock is therefore
held for at most the duration of one \code{git update-ref} round-trip plus the critical
section, without holding any OS-level file lock that could block unrelated operations. % [DEF]
\end{definition}
...
\code{refs/notes/praxis/audit}. % [THM]
Under the CAS lock of Definition~\ref{def:gitlock-op},
each append of a receipt record is a \emph{commit transition}: the new record is serialized
as NDJSON and appended to the notes tree under the commit OID it annotates. % [THM]
Because Git
commits are content-addressed (the commit hash is the BLAKE3 of its tree plus metadata),
the appended entry is immutable by Git's object model. % [THM]
The notes reference advances
atomically under the CAS guard, so two concurrent append attempts on the same ledger are
serialized by the acquire-fail branch of the CAS. % [THM]
\end{proposition}

\begin{proof}
A failing CAS (reference already exists) exits with a non-zero code before touching the
notes tree; only the lock holder proceeds. % [PROOF]
Within the critical section, the notes append
is a single \code{git notes add} followed by the reference-delete on the lock. % [PROOF]
If the
...
identifies a live critical section. % [PROOF]
\end{proof}
...
appended at a time} (no race). % [TEXT]
Together they give a ledger that is simultaneously
append-sequential (by the lock) and tamper-evident (by the chain). % [TEXT]
Violation of
sequentiality (two writers appending concurrently) would not break Theorem~\ref{thm:faithful};
it would produce a fork in the notes tree rather than a hash collision. % [BOUNDARY]
The CAS lock
prevents the fork; the chain prevents the tamper; neither subsumes the other. % [TEXT]
\end{remark}
...
cryptography does \emph{not} prove. % [TEXT]
Without it, a receipt chain becomes a laundering device:
tamper-evidence dressed up as a warrant of goodness. % [TEXT]
We state the boundary precisely. % [BOUNDARY]
...
authentically signed (Proposition~\ref{prop:intauth}). % [THM]
Then $\Ledger$ establishes exactly the
following, and no more: % [THM]
\begin{enumerate}[label=(\Alph*),leftmargin=*]
\item \textbf{Integrity.} The committed fields of every frame are as they were when minted;
no committed field was altered post hoc (up to $\varepsilon(\lambda)$). % [THM]
\item \textbf{Attribution.} Each artifact is committed to an admitted cause and a denial word,
and (if signed) to a keyholder. % [THM]
\item \textbf{Conformance-as-checked.} Each replayed lifecycle is a firing sequence of the
\emph{fixed} POWL model to fitness $\code{0x0001\_0000}$ (Proposition~\ref{prop:fitness}). % [THM]
\end{enumerate}
It does \textbf{not} establish: % [THM]
\begin{enumerate}[label=(\alph*),leftmargin=*]
\item \textbf{Obligation adequacy.} That the obligation battery $G$ was the \emph{right} set of
checks --- the chain commits $\dg(G)$ and the denial word, not the wisdom of choosing $G$. % [THM]
\item \textbf{World-fitness.} That the artifact is correct, safe, useful, or ethical in the
world --- $\muop$ is deterministic and bounded, not benevolent. % [THM]
\item \textbf{Semantic truth.} That the admitted observation \emph{meant} what it purported;
admission retracted onto a decidable sub-language (Rice quarantine), it did not decide meaning. % [THM]
\end{enumerate}
\end{theorem}
...
model is the correct model. % [PROOF]
...
inspects the world-consequences of $a=\muop(x)$ --- only that $a$'s digest is committed. % [PROOF]
Hence
...
weaker than truth. % [PROOF]
Each negative is therefore not entailed by (A)--(C). % [PROOF]
 \end{proof}
...
(Theorem~\ref{thm:scope}). % [THM]
The admissible claim is narrower and exact: ``this artifact was
...
is machinery; one that emits the broad claim is grandiosity. % [THM]
\end{corollary}

\begin{proof}
The broad claim entails (b), which Theorem~\ref{thm:scope} shows is independent of the verdict;
so the broad claim is not supported by the receipt. % [PROOF]
The narrow claim is exactly the conjunction
(A)$\wedge$(B)$\wedge$(C), each proven. % [PROOF]
Emitting only what is entailed is the keystone's
\code{docs-exceed-mechanism} invariant applied to cryptographic claims. % [PROOF]
 \end{proof}

\chapter{Conclusion}

We built the cryptography the keystone assumed. % [BOUNDARY]
A frame is a fixed-width, $128$-byte,
cache-aligned encoding of one manufacturing step whose $99$-byte hash body commits the full
payload digest, the denial word, and the transition identity (Chapter~\ref{ch:frame}). % [ORIENT]
A chain
folds frames by $h_{+}=\chainH(h_{-}\concat\body(\fr))$ into a rolling commitment computed by a
single, drift-free construction site (Chapter~\ref{ch:chain}). % [ORIENT]
From an explicit
...
cause (Chapter~\ref{ch:conservation}). % [BOUNDARY]
The staged validator does not merely detect tampering
but localizes it to the first divergent frame and names the fault class
(Chapter~\ref{ch:localize}). % [ORIENT]
Ed25519 signatures add self-contained authenticity, kept
distinct from integrity and made fail-closed (Chapter~\ref{ch:auth}). % [ORIENT]
Receipts accumulate into
a content-addressed lake with lossless OCEL~2.0 / PROV-O mappings and an index-scale query
direction (Chapter~\ref{ch:lake}). % [ORIENT]
The final theorem is the one a cryptographer must never omit: integrity is not virtue
(Chapter~\ref{ch:scope}). % [ORIENT]
A verifying, signed chain proves that a history is unaltered,
attributed, and conformant-as-checked --- and proves, with equal rigor, that it says nothing
about whether the obligations were wise, the artifact good, or the observation true. % [TEXT]
That
boundary is not a weakness of the receipt. % [BOUNDARY]
It is the discipline that lets a receipt be trusted
for exactly what it commits, at a scale where trusting it for more would be the first lie a
trillion agents tell each other. % [TEXT]

\begin{center}\itshape A receipt commits; it does not absolve. % [TEXT]
\end{center}
