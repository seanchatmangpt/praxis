...
Let $R\subseteq\Obs$ be the reachability fixpoint fact set. A frozen fact store builds an $8$-bit XOR filter over FNV-1a hashes of $R$. Index mapping uses the fastrange reduction:
...
Let $N$ be a safe \& sound workflow net, and let $\psi = \text{convert}(N)$ be its POWL 2.0 conversion. Then for any prefix trace length $k$:
\[
\mathcal{L}_k(N) \;=\; \mathcal{L}_k(\psi) \;=\; \mathcal{L}_k\bigl(\text{recompose}(\psi)\bigr).
\]
\end{theorem}
...
Let $A$ be a set of independent client accounts. Let $\text{realized}(a)$ be the revenue of account $a$ under a target stage, and let $\text{lawful\_targets}(a)$ be its evidence-gated target stages. The joint plan optimization decomposes linearly:
\begin{equation}\label{eq:mrr}
\max_{\text{joint plan}} \sum_{a\in A} \text{realized}(a)\;=\;\sum_{a\in A} \max_{t \in \text{lawful\_targets}(a)} \text{realized}(a, t).
\end{equation}
...
Let $(\mathcal{D},\mathcal{Ob})$ be a grounded PDDL problem with initial atom set $\bm
m_0$ and ground action set $T$. The \emph{Nemo Datalog program} $\Pi$ for this problem
has one fact per atom in $\bm m_0$ and one rule per ground action $t\in T$:
\[
...
include $p$, i.e.\ $p$ is reachable from $\bm m_0$ in the Petri-net sense of
Chapter~\ref{ch:polytope}.
\end{proposition}
...
action cannot contribute to any plan reachable from $\bm m_0$. This is the
...
(e.g.\ for the initial atom set $\bm m_0$) is a single hash over $4(k+1)$ bytes. The
